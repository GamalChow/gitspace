...
# 保存ip地址配置,供出错时参考
store_ip_info() {
	echo -e "count:$node_count\nvip:$old_vip:$new_vip:$new_mask\ngateway:$new_gateway">$input_record
	
	#数组转换成字符串保存到文件中
	oip_str=$(printf ":%s" "${old_ip[@]}")
	nip_str=$(printf ":%s" "${new_ip[@]}")

	echo -e "old_ip$oip_str\nnew_ip$nip_str">>$input_record
}
...
setup_locale_ipaddr(){
	...
	log_info "正在修改IP地址：$old_ip1,请连接新地址【$new_ip1】..."
