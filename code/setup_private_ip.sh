...
input_record="/tmp/.ip.record.private_ip"
...
# 保存ip地址配置,供出错时参考
store_ip_info() {
	echo -e "count:$node_count\nmask:$new_mask">$input_record

	for i in "${!old_ip[@]}"; do
		if [ ${old_ip[$i]} ]; then
			echo "节点$i:${old_ip[$i]}:${new_ip[$i]}">>$input_record
		fi
	done
...
# 更新hosts和nodes配置文件
update_hosts_nodes() {
	log_info "修改配置hosts和nodes配置文件"

	for i in "${!old_ip[@]}"; do
		if [ ${old_ip[$i]} ];then
			for j in "${!old_ip[@]}"; do
				if [ ${old_ip[$j]} ]; then
					local cmd="sed -i 's/${old_ip[$j]}/${new_ip[$j]}/g' /etc/hosts /etc/ctdb/nodes"
...
# 更新数据库中private_ip地址
update_database() {
	log_info "更新数据库private_ip设置..."
    
	for i in "${!old_ip[@]}"; do
		if [ ${old_ip[$i]} ]; then
			echo -e "rqlite -u admin:sysadmin << EOF\nupdate hardware_device set private_ip='${new_ip[$i]}' where private_ip='${old_ip[$i]}'\nexit docker\nEOF">update_ipaddress
...
restart_ha_service() {
	...
	local cmd="systemctl restart glusterd && sleep 10 && systemctl restart ctdb"
...
main() {
	...
	# 4.更新配置文件/etc/hosts、/ect/ctdb/nodes
	update_hosts_nodes
